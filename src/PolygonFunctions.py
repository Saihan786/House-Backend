...
def findAdjacentLine(polygon):
    ...
    if adjacentlineindex >= len(lines):
        adjacentlineindex = 0
...
def find_path(
    coords, startpoint, endpoint, longestline, pathIsHorizontal, shouldContainLL
):
    ...
    if (shouldContainLL and not containsLongestLine) or (
        ...
        startpointfirst = (
            rcoords[startpointfirst:-1] + [rcoords[-1]] + rcoords[0:startpointfirst]
        )
...
def findLinePaths(polygon, showPaths=False):
    ...
    mparallel, c, isV = LineFunctions.lineEQ(
        longestline[linep1idx], longestline[linep2idx]
    )
    mperp = -1 / mparallel
...
def rotatePolygon(resultLine, polygon, showRotation=False):
    ...
    resultAngle, changeAngle = LineFunctions.findAngle(
        resultLine
    ), LineFunctions.findAngle(polygonLine)
    polygon = affinity.rotate(
        polygon, resultAngle - changeAngle, origin="centroid", use_radians=True
    )
    polygonLine = LineString([polygon.exterior.coords[0], polygon.exterior.coords[1]])
    ...
    if showRotation:
        ...
        print(
            "The actual rotated angle is",
            np.rad2deg(LineFunctions.findAngle(polygonLine)),
        )
...
def moveToOrigin(polygon, showTranslation=False):
    ...
    shiftcoords = [
        (coord[X] - leftmost, coord[Y] - bottom)
...
def centerAtOrigin(polygon, showTranslation=False):
    ...
    *****ONLY WORKS FOR POLYGONS*****
    ...
    shiftcoords = [
        (coord[X] - center.x, coord[Y] - center.y)
        for coord in polygon.exterior.coords[:-1]
    ]
    ...
    if showTranslation:
        ...
        geopandas.GeoSeries([pold.exterior, Point(pold.centroid)]).plot(
            ax=ax, color="red"
        )
        geopandas.GeoSeries([pnew.exterior, Point(pnew.centroid)]).plot(
            ax=ax, color="green"
        )
