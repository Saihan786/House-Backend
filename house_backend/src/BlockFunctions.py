...
class UnitPolygon:
    ...
    def __move_single_polygon(
        ...
        try:
            for corner in polygon.exterior.coords[:-1]:
                corners.append(
                    (
                        corner[X] + blockpoint.coords[0][X],
    ...
    def move(self, blockpoint, polygon_to_fit_inside=None):
        ...
        if self.type == "polygon" and not bp_is_gdf:
            return self.__move_single_polygon(
                self.item_to_plot, blockpoint, polygon_to_fit_inside
            )
            ...
            try:
                ...
                for polygon in copyblock.geometry:
                    ...
                    moved_polygon = self.__move_single_polygon(
                        polygon=polygon,
                        blockpoint=bp,
                        polygon_to_fit_inside=polygon_to_fit_inside,
                    )
            ...
            result = self.__move_single_geometry(
                blockpoint=blockpoint,
                geometry=copyblock.geometry,
                polygon_to_fit_inside=polygon_to_fit_inside,
            )
    ...
    def move_to(self, up):
        ...
        try:
            leq = LineFunctions.lineEQ(
                (init_point.x, init_point.y), (prev_point.x, prev_point.y)
            )
        except:
            raise Exception
        ...
        if shape_type == "polygon":
            if self.type == "polygon":
                final_point = LineFunctions.point_from_distance(
                    leq, init_point, dist(self, up)
                )
            ...
            if self.type == "polygon":
                ...
                for p in self.item_to_plot.geometry:
                    distances_to_up = geopandas.GeoSeries.distance(
                        up.item_to_plot, p
                    )
                ...
                for d in distances:
                    new_points.append(
                        Point(
                            LineFunctions.point_from_distance(
                                leq, init_point, d
                            )
                        )
                    )
    ...
    def intersects(self, shape, shape_type="polygon"):
        ...
        if shape_type == "polygon":
            ...
            if self.type == "polygon":
                ...
                return True in list(
                    self.item_to_plot.intersects(shape.item_to_plot)
                )
        else:
            print("Unrecognised shape_type when intersecting UnitPolygon.")
    ...
    def rotate(self, line, should_be_centered=True):
        ...
        if self.type == "polygon":
            self.item_to_plot = PolygonFunctions.rotatePolygon(
                LineString(line), self.item_to_plot, showRotation=False
            )
        elif self.type == "gdf":
            InputBlocks.dxf_parallel_to_ll(
                dxf=self.item_to_plot, center_at_origin=False, resultline=line
            )
    ...
    for x in range(-1 + len(block_ups_as_rows)):
        ...
        for y in range(len(block_ups_as_rows[x])):
            ...
            if keepBlock:
                ...
                row.append(
                    UnitPolygon(type=block_up.type, item_to_plot=new_block)
                )
...
def initPlot(
    ...
    if showInit:
        geopandas.GeoSeries(
            [db.exterior for row in distinctblock_ups for db in row]
...
def plotNewBlocks(
    ...
    if current_plot:
        ...
        distinctblocks = filter_blocks(
            block_ups_as_rows, smallest_up, replaceSmall=True
        )
        print("passed distinct")

    if showBlocks:
        geopandas.GeoSeries(
            [block.exterior for row in distinctblocks for block in row]
        ).plot(ax=ax, color="green")
    return distinctblocks
